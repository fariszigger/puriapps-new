...
@section('content')
    ...
    <div x-data="dashboardStats()" x-init="init()">
        <div class="flex flex-wrap items-center gap-3 mb-4">
            ...
            @can('view credit-disbursements')
                @if(auth()->user()->hasRole('AO'))
                    <!-- Total Pencairan for AO -->
                    <div class="ml-auto inline-flex items-center bg-gray-50/90 backdrop-blur-sm border border-emerald-200/60 shadow-sm px-4 py-1.5 rounded-full gap-2">
                        ...
                        <div class="flex items-center gap-2 pr-2">
                            ...
                            <span class="text-[17px] font-black text-emerald-700 leading-none tracking-tight" x-text="'Rp ' + Number(stats.totalDisbursement || 0).toLocaleString('id-ID') + ' / Rp ' + Number(stats.totalLimit || 0).toLocaleString('id-ID') + ' (' + (stats.totalLimit > 0 ? ((stats.totalDisbursement / stats.totalLimit) * 100).toFixed(1) : 0) + '%)'">Rp {{ number_format($totalDisbursement ?? 0, 0, ',', '.') }} / Rp {{ number_format($totalLimit ?? 0, 0, ',', '.') }} ({{ ($totalLimit ?? 0) > 0 ? number_format(($totalDisbursement / $totalLimit) * 100, 1) : 0 }}%)</span>
                        </div>
                    </div>
                @endif
            @endcan
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
            <template x-for="card in cards" :key="card.key">
                <div @click="toggleChart(card.key)"
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-600" x-text="card.label === 'Total Target' ? 'Limit Pencairan' : card.label"></p>
    ...
    <!-- Daftar Janji Bayar (AO Only) - Moved to top for priority -->
    ...
    @can('view warning-letters')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Daftar Pencairan Card -->
            @can('view credit-disbursements')
                <a href="{{ route('credit-disbursements.index') }}" class="block p-6 bg-white/40 backdrop-blur-md rounded-xl border border-white/50 shadow-xl hover:bg-white/50 transition-all duration-300 transform hover:-translate-y-1 group">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <p class="font-normal text-gray-700">Kelola dan pantau pencairan kredit nasabah.</p>
                </a>
            @endcan
