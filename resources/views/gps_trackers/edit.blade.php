...
@section('breadcrumb-items')
    ...
    <li class="inline-flex items-center">
        <div class="flex items-center">
            ...
            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Edit GPS</span>
        </div>
    </li>
@endsection
...
@section('content')
    <div class="px-4 py-6 md:px-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">Edit GPS Tracker</h1>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-2xl">
            <form action="{{ route('gps-trackers.update', $gps_tracker->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Nama/Label GPS</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $gps_tracker->name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('name') border-red-500 @enderror" placeholder="Contoh: Kendaraan Operasional 01" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="imei" id="imei" value="{{ old('imei', $gps_tracker->imei) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 font-mono @error('imei') border-red-500 @enderror" placeholder="Contoh: 867010070425674" required>
                ...
                <div class="mb-4">
                    ...
                    <select name="user_id" id="user_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $gps_tracker->user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }} ({{ $user->code }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="status" id="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="active" {{ old('status', $gps_tracker->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $gps_tracker->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center justify-end gap-3 mt-8 border-t pt-6 bg-gray-50 -mx-6 -mb-6 px-6 pb-6">
                    ...
                    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-bold rounded-lg text-sm px-5 py-2.5 shadow-lg hover:shadow-blue-500/30">Perbarui GPS Tracker</button>
                </div>
            </form>
        </div>
    </div>
