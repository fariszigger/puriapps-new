<div>
            ...
            <h3 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                ...
                @if($viewMode === 'period' && $filterMonth && $filterMonthEnd)
                    <span class="text-sm font-normal text-gray-500">
                        — {{ \Carbon\Carbon::parse($filterMonth . '-01')->translatedFormat('F Y') }} s/d
                        {{ \Carbon\Carbon::parse($filterMonthEnd . '-01')->translatedFormat('F Y') }}
                    </span>
                @elseif($filterMonth)
                    <span class="text-sm font-normal text-gray-500">
        ...
        @if($aoSummary->count() > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @foreach($aoSummary as $summary)
                    @if($summary['is_hrd'] ?? false)
                        {{-- HRD Special Card --}}
                        <div
                            class="bg-purple-50/60 backdrop-blur-sm rounded-xl border border-purple-200/50 p-4 shadow-sm hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between mb-2">
                                <div>
                                    <p class="text-sm font-bold text-purple-900">{{ $summary['code'] }}</p>
                                    <p class="text-[10px] font-bold text-purple-400 uppercase tracking-wider">{{ $summary['name'] }}
                                    </p>
                                </div>
                            ...
                            <p class="text-lg font-black text-purple-900 mb-1">Rp
                                {{ number_format($summary['total_amount'], 0, ',', '.') }}</p>
                            <p class="text-[10px] text-purple-500">
                                {{ $summary['total_count'] }} pencairan
                            </p>
                        </div>
                    @else
                        {{-- Regular AO Card --}}
                        <div
                            class="bg-white/60 backdrop-blur-sm rounded-xl border border-white/50 p-4 shadow-sm hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between mb-2">
                                <div>
                                    <p class="text-sm font-bold text-gray-900">{{ $summary['code'] }}</p>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{{ $summary['name'] }}
                                    </p>
                                </div>
                                <span
                            ...
                            <p class="text-lg font-black text-gray-900 mb-1">Rp
                                {{ number_format($summary['total_amount'], 0, ',', '.') }}</p>
                            ...
                            <p class="text-[10px] text-gray-500">
                                {{ $summary['total_count'] }} pencairan · Target: Rp
    ...
    <div class="bg-white/50 backdrop-blur-md rounded-2xl border border-white/50 p-4 shadow-xl mb-8 space-y-4">
        <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
                ...
                <div class="relative w-full sm:w-72 group">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <svg class="w-4 h-4 text-emerald-500 transition-transform group-focus-within:scale-110"
            ...
            <div
                class="flex flex-wrap items-center justify-center lg:justify-end gap-3 w-full lg:w-auto border-t lg:border-t-0 pt-4 lg:pt-0 border-gray-100">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @elseif($viewMode === 'period')
                        <div class="flex items-center gap-2">
                            <div class="relative min-w-[140px]">
                                <span
                                    class="absolute -top-4 left-2 text-[8px] font-bold text-emerald-600 uppercase">Dari</span>
                                <input type="month" wire:model.live="filterMonth"
                                    class="w-full pl-4 pr-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 focus:ring-2 focus:ring-emerald-500/50 transition-all cursor-pointer">
                            </div>
                            <div class="relative min-w-[140px]">
                                <span
                                    class="absolute -top-4 left-2 text-[8px] font-bold text-emerald-600 uppercase">Sampai</span>
                ...
                <select wire:model.live="filterAo"
                    ...
                    @foreach($aoUsers as $user)
                        @php $isHrd = $user->roles->contains('name', 'HRD'); @endphp
                        @if(!$isHrd)
                            <option value="{{ $user->id }}">[{{ $user->code }}] {{ $user->name }}</option>
                        @endif
                    @endforeach
                    <option value="hrd">── [HRD] Karyawan ──</option>
                </select>
            ...
            <tbody>
                @forelse($disbursements as $item)
                    <tr wire:key="disbursement-{{ $item->id }}"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="px-2 py-1 rounded bg-gray-100/80 text-gray-700 font-bold uppercase text-xs tracking-wider">
                                {{ $item->user->code ?? '-' }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            ...
                            @if($status == 'aktif' || $status == 'active' || empty($status))
                                <span
                                    class="px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-bold uppercase tracking-wider">Aktif</span>
                            @else
                                <span
                                    class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-bold uppercase tracking-wider">Lunas</span>
                            @endif
                        </td>
                        @if(auth()->user()->can('edit credit-disbursements') || auth()->user()->can('delete credit-disbursements'))
                            <td class="px-6 py-4">
                ...
                @empty
                    <tr class="bg-white/40 border-b border-white/40">
                        <td colspan="{{ (auth()->user()->can('edit credit-disbursements') || auth()->user()->can('delete credit-disbursements')) ? 11 : 10 }}"
                            ...
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <div class="max-w-xs mx-auto">
                                    ...
                                    <span class="text-sm text-gray-400 font-medium">Belum ada data pencairan untuk periode
                                        ini.</span>
                                </div>
                            </div>
                        </td>
