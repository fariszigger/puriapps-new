<div>
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
        <div class="w-full md:w-1/2">
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="search" type="search" id="customer-search"
                    ...
                    placeholder="Cari berdasarkan Nama, Email, Phone, atau Alamat..">
            </div>
        </div>
                ...
                @forelse($customers as $customer)
                    <tr wire:key="customer-{{ $customer->id }}"
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            {{ $customers->total() - (($customers->currentPage() - 1) * $customers->perPage()) - $loop->index }}
                        </td>
                        ...
                        <td class="px-6 py-4">{{ $customer->user->code ?? '-' }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <a href="{{ route('customers.print', $customer->id) }}" target="_blank"
                                ...
                                @can('delete customers')
                                    <button type="button" onclick="confirmDelete({{ $customer->id }}, '{{ $customer->name }}')"
    ...
    <script>
        function confirmDelete(id, name) {
            Swal.fire({
                title: 'Yakin Data ' + name + ' untuk Dihapus ?',
                text: "Data akan disimpan di folder sampah (Soft Delete)",
                icon: 'warning',
