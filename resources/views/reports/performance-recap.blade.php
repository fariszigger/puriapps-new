                            ...
                            @foreach($aoData['cross_period_fulfilled'] as $i => $cp)
                                <tr>
                                    ...
                                    <td class="text-center font-semibold align-middle" style="background-color: #eff6ff !important;">
                                        {{ formatIndonesianDate(\Carbon\Carbon::parse($cp->created_at)) }}
                                        <br>
                                    ...
                                    <td>
                                        ...
                                        @if($cp->tanggal_janji_bayar)
                                            — {{ formatIndonesianDate(\Carbon\Carbon::parse($cp->tanggal_janji_bayar)) }}
                                        @endif
                                        @if($cp->jumlah_pembayaran)
                                            <br><span class="text-[9px]">Rp {{ number_format($cp->jumlah_pembayaran, 0, ',', '.') }}</span>
                                        @endif
                                        <br><span style="color:#16a34a;font-weight:bold;font-size:9px">✓ SUDAH BAYAR Rp {{ number_format($cp->jumlah_bayar_fulfilled ?? 0, 0, ',', '.') }} pd. {{ formatIndonesianDate(\Carbon\Carbon::parse($cp->janji_bayar_fulfilled_at)) }}</span>
                                    </td>
                                </tr>
                            @endforeach
